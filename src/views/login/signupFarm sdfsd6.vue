...
<div>
    ...
    <v-form ref="form" lazy-validation @submit.prevent="SignupForm">
        <v-row>
            <v-col cols="12">
            <v-text-field v-model="f_name" label="이름*" :rules="f_name_rule" required></v-text-field>
            </v-col>
            <v-col cols="12">
                <v-text-field v-model="f_email" label="이메일*" :rules="emailRules"
                    :disabled="state == 'ins' ? false : true" required></v-text-field>
            </v-col>

            <button class="login-form__btn_right" @click="existEmail(f_email)">이메일 중복 검사</button>

            <v-col cols="12">
                <v-text-field v-model="f_passwd" label="비밀번호*" type="password" :rules="f_passwd_rule"></v-text-field>
            </v-col>
            <v-col cols="12">
                <v-text-field v-model="f_passwd_chk" label="비밀번호 확인*" type="password" :rules="f_passwd_rule2">
                </v-text-field>
            </v-col>
            <v-col cols="12">
                <v-text-field v-model="f_phonenum" label="핸드폰 번호" :rules="f_phonenum_rule"></v-text-field>
            </v-col>
            <!-- <v-col cols="12">
...
export default {
    ...
    data() {
        return {
            ...
            f_passwd_rule2: [
                v => this.state === 'ins' ? !!v || '패스워드는 필수 입력사항입니다.' : true,
                v => !(v && v.length >= 30) || '패스워드는 30자 이상 입력할 수 없습니다.',
                v => v === this.f_passwd || '패스워드가 일치하지 않습니다.'
            ],
            user_auth: '',
            user_auth_rule: [
                v => !!v || '권한은 필수 선택 사항입니다.'
    ...
    methods:{
        existEmail(f_email) {
            this.$store.dispatch('existEmail', f_email)
            console.log(f_email);
            if(f_email == f_email){
        ...
        async signupfarm() {
            ...
            if(!validate){
                ...
                console.log();
                axios.post('/api/signupFarmMember', { f_name: this.f_name, f_email: this.f_email, f_passwd: this.f_passwd, f_phonenum: this.f_phonenum})
                    .then(res => {
                        console.log(res);
                        if (res.data == 0) {
                            alert("회원가입 실패..")
